...
export async function getTranslationFromCache(
  ...
  try {
    ...
    if (error || !data) {
      console.log(
        `🔍 Cache miss for: "${text.substring(0, 30)}..." → ${targetLanguage}`,
      );
      return null;
    }

    console.log(
      `✅ Cache hit for: "${text.substring(0, 30)}..." → ${targetLanguage} (${data.translation_engine})`,
    );
...
export async function saveTranslationToCache(
  ...
  try {
    ...
    console.log(
      `💾 Saving to cache: "${text.substring(0, 30)}..." → ${targetLanguage} (${engine})`,
    );
    ...
    if (error) {
      console.error(
        `❌ Error saving translation to cache (${totalTime}ms):`,
        error,
      );
      ...
      if (error.code === '23505') {
        ...
        const existingCache = await getTranslationFromCache(
          text,
          targetLanguage,
        );
...
export async function getCacheStats(): Promise<{
  ...
  try {
    ...
    data.forEach((entry) => {
      byEngine[entry.translation_engine] =
        (byEngine[entry.translation_engine] || 0) + 1;
      byLanguage[entry.target_language] =
        (byLanguage[entry.target_language] || 0) + 1;
...
export async function cleanupLowUsageCache(
...
export async function getCacheSize(): Promise<{
  ...
  try {
    ...
    data.forEach((entry) => {
      totalBytes +=
        (entry.original_text?.length || 0) +
        (entry.translated_text?.length || 0) +
        100; // 메타데이터 추정
    });
...
// 🆕 스마트 캐시 정리 (크기 기반)
export async function smartCacheCleanup(
  ...
  if (cacheSize.estimatedSizeMB <= maxSizeMB) {
    console.log(
      `✅ Cache size (${cacheSize.estimatedSizeMB.toFixed(2)}MB) is within limit (${maxSizeMB}MB)`,
    );
    return 0;
  }

  console.log(
    `⚠️ Cache size (${cacheSize.estimatedSizeMB.toFixed(2)}MB) exceeds limit (${maxSizeMB}MB)`,
  );
