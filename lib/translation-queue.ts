...
): Promise<{ text: string; quality: number } | null> {
  try {
    ...
    if (data.candidates && data.candidates[0] && data.candidates[0].content) {
      ...
      if (candidate.finishReason === 'MAX_TOKENS') {
        console.log(
          '⚠️ Gemini individual response was truncated due to MAX_TOKENS',
        );
        return null;
      }
      ...
      if (
        ...
        if (translatedText.startsWith('{') && translatedText.endsWith('}')) {
          try {
            ...
            console.log(
              'Individual Gemini response is not valid JSON, using as-is',
            );
          }
        }
        ...
        console.log(
          '❌ Gemini individual response missing content.parts:',
          candidate.content,
        );
      }
    }
...
): Promise<Record<string, { text: string; quality: number }> | null> {
  try {
    const geminiApiKey = process.env.GEMINI_API_KEY;
    if (!geminiApiKey) {
      console.log(
        'Gemini API key not found, skipping Gemini batch translation',
      );
      return null;
    }

    console.log(
      `🔑 Gemini API key found (${geminiApiKey.substring(0, 10)}...)`,
    );

    // 지원되는 언어만 필터링
    const supportedLanguages = targetLanguages.filter(
      (lang) => GEMINI_LANGUAGE_NAMES[lang],
    );
    console.log(
    ...
    if (supportedLanguages.length === 0) {
      console.log(
        '❌ No supported languages found for Gemini batch translation',
      );
      return null;
    }
    ...
    const languageList = supportedLanguages
      .map((lang) => `${lang}: ${GEMINI_LANGUAGE_NAMES[lang]}`)
      .join(', ');
        ...
        body: JSON.stringify({
          ...
          generationConfig: {
            ...
            maxOutputTokens: Math.max(
              Math.ceil(text.length * supportedLanguages.length * 4),
              1500,
            ), // 토큰 제한 증가
          },
        }),
    ...
    if (
      ...
      try {
        ...
        if (Object.keys(result).length > 0) {
          console.log(
            `✅ Gemini batch translation parsed successfully: ${Object.keys(result).join(', ')}`,
          );
          return result;
        }
      } catch (parseError) {
        ...
        try {
          ...
          if (jsonMatch) {
            ...
            if (Object.keys(result).length > 0) {
              console.log(
                `✅ Gemini batch translation extracted via regex: ${Object.keys(result).join(', ')}`,
              );
              return result;
            }
          }
...
async function performTranslation(
  text: string,
  targetLanguage: string,
): Promise<{ text: string; engine: string; quality: number }> {
  // 1단계: Gemini 시도 (최고 품질)
...
export async function performBatchTranslation(
  ...
  const results: Record<
    string,
    { text: string; engine: string; quality: number }
  > = {};

  // 1단계: Gemini 배치 번역 시도 (55% 비용 절약)
  try {
    console.log(
      `🎯 Attempting Gemini batch translation for: "${text.substring(0, 50)}..." → [${targetLanguages.join(', ')}]`,
    );
    const batchResult = await translateWithGeminiBatch(text, targetLanguages);
    if (batchResult && Object.keys(batchResult).length > 0) {
      console.log(
        `🚀 Gemini batch translation succeeded for ${Object.keys(batchResult).length}/${targetLanguages.length} languages`,
      );
  ...
  if (failedLanguages.length > 0) {
    console.log(
      `🔄 Falling back to individual translation for ${failedLanguages.length} languages`,
    );
  ...
  for (const [language, result] of Object.entries(batchResults)) {
    try {
      ...
      if (cacheId) {
        ...
        console.log(
          `✅ Cached translation: "${text.substring(0, 30)}..." → ${language} (${result.engine})`,
        );
      }
    } catch (error) {
...
class TranslationQueueManager {
  ...
  addJob(job: TranslationJob): void {
    ...
    console.log(
      `📝 Added job for "${job.text.substring(0, 30)}..." → ${job.targetLanguage} (total languages: ${textGroup.languages.size})`,
    );
  ...
  private scheduleProcessing(textKey: string): void {
    ...
    console.log(
      `⏰ Scheduling batch processing for "${textGroup.text.substring(0, 30)}..." with ${languageCount} languages in ${delay}ms`,
    );
  ...
  private async processBatch(textKey: string): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        const batchResults = await performBatchTranslation(
          textGroup.text,
          languageArray,
        );
        ...
        for (const [language, result] of Object.entries(batchResults)) {
          ...
          console.log(
            `✅ Batch translated "${textGroup.text.substring(0, 30)}..." → ${language} using ${result.engine}`,
          );
        }
        ...
        console.error(
          `❌ Batch translation failed for "${textGroup.text.substring(0, 50)}...":`,
          error,
        );

        // 실패시 개별 처리로 폴백
        console.log(
          `🔄 Falling back to individual translations for ${languageArray.length} languages`,
        );
        ...
        for (const language of languageArray) {
          try {
            ...
            console.error(
              `❌ Individual translation failed for ${language}:`,
              individualError,
            );
          }
        }
  ...
  private async updateTranscriptStatus(jobs: TranslationJob[]): Promise<void> {
    ...
    try {
      ...
      if (error) {
        ...
        console.log(
          `✅ Updated ${transcriptIds.length} transcript(s) status to completed`,
        );
      }
    } catch (error) {
  ...
  getQueueStats(): Record<string, number> {
    ...
    for (const [textKey, textGroup] of this.textQueues) {
      ...
      stats[
        `"${textKey.substring(0, 20)}..."${textKey.length > 20 ? '...' : ''}`
      ] = textGroup.languages.size;
    }
...
function calculatePriority(targetLanguage: string, sessionId?: string): number {
  ...
  if (PRIORITY_LANGUAGES.includes(targetLanguage)) {
    priority +=
      (PRIORITY_LANGUAGES.length - PRIORITY_LANGUAGES.indexOf(targetLanguage)) *
      2;
  }
