...
export default function MySessionsPage() {
  ...
  const renderDaysBadge = (session: SavedSession) => {
    ...
    if (daysRemaining <= 3) {
      return <Badge className='bg-orange-100 text-xs text-orange-800'>D-{daysRemaining}</Badge>
    }

    if (daysRemaining <= 7) {
      return <Badge className='bg-yellow-100 text-xs text-yellow-800'>D-{daysRemaining}</Badge>
    }

    return <Badge className='bg-green-100 text-xs text-green-800'>D-{daysRemaining}</Badge>
  }
      ...
      <div className='space-y-4'>
        {loading ? (
          ...
          <div className='py-12 text-center'>
            ...
            <p className='mt-1 text-sm text-gray-400'>Start a session or join one to see it here</p>
          </div>
        ) : (
      ...
      {userProfile?.subscription_status !== 'premium' && (
        <Card className='mt-8 border-purple-200 bg-purple-50'>
          <CardHeader>
            ...
            <CardDescription>Get unlimited access to all your sessions and premium features</CardDescription>
          </CardHeader>
          <CardContent>
