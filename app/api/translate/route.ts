...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      '2️⃣ Cache miss - generating mock translation and queuing job...',
    );
    ...
    console.log(
      `📋 Translation job ${jobId} queued with priority ${priority} for "${text.substring(0, 30)}..."`,
    );
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(
      `🔍 Checking translation status for: "${text.substring(0, 30)}..." → ${targetLanguage}`,
    );
    ...
    console.log(
      `⏳ Translation still in progress for: "${text.substring(0, 30)}..." → ${targetLanguage}`,
    );
    return NextResponse.json({
      completed: false,
