  ...
  try {
    ...
    switch (type) {
      ...
      case 'transcript':
        ...
        if (!session) {
          console.error(
            `❌ Session ${sessionId} not found for transcript update`,
          );
        ...
        if (!cleanedTranscript || cleanedTranscript.length < 3) {
          console.log(
            `⚠️ Skipping empty or too short transcript: "${cleanedTranscript}"`,
          );
          return NextResponse.json({
            success: true,
        ...
        if (session.fullTranscript.includes(cleanedTranscript)) {
          console.log(
            `⚠️ Duplicate transcript detected, skipping: "${cleanedTranscript.substring(0, 30)}..."`,
          );
          return NextResponse.json({
            success: true,
        ...
        if (!isPartial && cleanedTranscript) {
          ...
          console.log(
            `📝 Final transcript added to session ${sessionId}:`,
            cleanedTranscript,
          );
          ...
          console.log(
            `💾 Inserting transcript to DB: "${cleanedTranscript.substring(0, 50)}..."`,
          );
          ...
          if (insertError) {
            console.error(
              `❌ DB insert error (${dbInsertTime}ms):`,
              insertError,
            );
            return NextResponse.json(
          ...
          console.log(
            `✅ Transcript saved (id): ${data?.[0]?.id} - DB insert: ${dbInsertTime}ms`,
          );
          ...
          console.log(
            `🔄 Translation status updated to 'processing' (${statusUpdateTime}ms)`,
          );

          // 영어 제외한 우선순위 언어들
          const targetLanguages = PRIORITY_LANGUAGES.filter(
            (lang) => lang !== 'en',
          );
          ...
          try {
            ...
            const cacheIds = await saveBatchTranslationsToCache(
              cleanedTranscript,
              batchResults,
            );
            ...
            console.log(
              `💾 Translation cache saved in ${cacheTime}ms for ${Object.keys(cacheIds).length} languages`,
            );
      ...
      case 'end':
        ...
        console.log(
          `🧹 Session ${sessionId} memory cleanup (${ended ? 'removed' : 'not found'})`,
        );
