...
export default function SessionTranscriptPage() {
  ...
  useEffect(() => {
    const loadSessionTranscript = async () => {
      ...
      try {
        ...
        setError(
          `Failed to load transcript: ${error instanceof Error ? error.message : 'Unknown error'}`,
        );
      } finally {
        setLoading(false);
      }
  ...
  const translateSummary = useCallback(
    async (summaryText: string, targetLang: string) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log(
            '🔔 Transcript page realtime update:',
            payload.eventType,
            payload.new,
          );
          ...
          if (payload.eventType === 'INSERT' && payload.new) {
            ...
            if (newTranscript.translation_status !== 'completed') {
              console.log(
                `⏳ Skipping transcript (status: ${newTranscript.translation_status})`,
              );
              return;
            }
  ...
  const translateText = useCallback(
    async (text: string, targetLang: string): Promise<string> => {
      ...
      try {
        console.log(
          `🌍 Translating: "${text.substring(0, 30)}..." → ${targetLang}`,
        );
          ...
          for (let i = 0; i < 5; i++) {
            ...
            try {
              const statusResponse = await fetch(
                `/api/translate?text=${encodeURIComponent(text)}&targetLanguage=${targetLang}`,
              );
              if (statusResponse.ok) {
                ...
                if (statusResult.completed) {
                  ...
                  console.log(
                    `✅ Real translation received: ${statusResult.engine}`,
                  );
                  break;
                }
              }
    ...
    async (type: 'original' | 'translation', event?: React.MouseEvent) => {
      ...
      const textContent = transcript
        .map((line, index) => {
          const text =
            type === 'original'
              ? line.original_text
              : translatedTexts[line.id] || line.original_text;
      ...
      try {
        ...
        addToast({
          ...
          title: '복사 실패',
          message:
            '클립보드 접근이 실패했습니다. 브라우저 설정을 확인해주세요.',
          duration: 5000,
        });
      }
    },
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="font-medium text-gray-900">Failed to Load Transcript</p>
  ...
  return (
    ...
    >
      {/* Header */}
      <header
        className={`sticky top-0 z-40 border-b ${darkMode ? 'border-gray-700 bg-gray-800' : 'bg-white'}`}
      >
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                ...
                <ChevronRight
                  className={`h-4 w-4 transition-transform ${showTranslation ? 'rotate-90' : ''}`}
                />
              </Button>
              <Button
                variant="ghost"
      ...
      {showSettings && (
        <div
          className={`border-b ${darkMode ? 'border-gray-700 bg-gray-800' : 'bg-white'} p-4`}
        >
          <div className="container mx-auto">
            <div className="grid gap-4 md:grid-cols-4">
              ...
              <div className="space-y-2">
                <div
                  className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'} rounded p-3 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}
                >
                  <div className="space-y-1">
                    <div>
                      📝 <strong>Completed Session</strong>
                    </div>
                    <div>• {transcript.length} transcript lines</div>
                    {showTranslation && (
                      <>
                        <div>
                          • Target: {selectedLang?.flag} {selectedLang?.name}
                        </div>
                        <div>
                          • Translating: {translatingIds.size} remaining
                        </div>
                      </>
                    )}
                  </div>
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="mx-auto max-w-4xl">
          ...
          {session?.status === 'ended' && (
            <div
              className={`mb-8 rounded-lg border-2 border-dashed p-6 ${
                darkMode
            ...
            >
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <span>
                        AI-generated summary based on {transcript.length}{' '}
                        transcript lines
                      </span>
                    </p>
                  </div>
              ...
              >
                {summaryLoading && (
                  <div className="flex items-center space-x-3 py-8 text-center">
                    ...
                    <div>
                      ...
                      >
                        전체 transcript를 분석하여 {session.category} 분야에
                        맞는 요약을 생성하고 있습니다.
                      </p>
                    </div>
                  </div>
                ...
                {summaryError && (
                  <div className="py-8 text-center">
                    <div className="mb-2 text-red-500">⚠️ 요약 생성 실패</div>
                    <p
                      ...
                      {showTranslation && selectedLanguage !== 'en' ? (
                        <span
                          dangerouslySetInnerHTML={{
                            __html: translatedSummary || summary,
                          }}
                        />
                      ) : (
          ...
          {transcript.length === 0 ? (
            <div
              className={`py-12 text-center ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}
            >
              ...
              {transcript.map((line) => (
                <div key={`text-only-${line.id}`}>
                  ...
                  {showTranslation && translatedTexts[line.id] && (
                    <div
                      className={`border-l-2 pl-4 leading-relaxed italic ${
                        darkMode
            ...
            <div className="space-y-3">
              {transcript.map((line, index) => (
                <div key={line.id} className="group">
                  ...
                  {showTimestamps && (
                    <div
                      className={`mb-1 flex items-center space-x-2 text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}
                    >
                  ...
                  {showTranslation && (
                    ...
                    >
                      {translatingIds.has(line.id) ? (
                        ...
                        translatedTexts[line.id] ||
                        `[${selectedLang?.name}] ${line.original_text}`
                      )}
                    </div>
                  )}
                </div>
