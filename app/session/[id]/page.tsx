...
export default function SessionPage() {
  ...
  const getTabContent = (type: 'original' | 'translated') => {
    return transcript.map((line) => (
      <div key={`${line.id}-${type}`} className={`mb-3 rounded-lg p-3 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
        {showTimestamps && (
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'dark bg-gray-900' : 'bg-gray-50'}`}>
        ...
        <div className='px-4 py-3'>
          <div className='flex items-center justify-between'>
            <div className='flex items-center space-x-3'>
              <div className={`h-3 w-3 rounded-full ${darkMode ? 'bg-green-500' : 'bg-red-500'} animate-pulse`}></div>
              <div>
            ...
            <div className='flex items-center space-x-2'>
              <Button variant='ghost' size='sm' onClick={saveSession} className={isSaved ? 'text-blue-600' : ''}>
      ...
      {showSettings && (
        <div className={`border-b ${darkMode ? 'border-gray-700 bg-gray-800' : 'bg-white'} p-4`}>
          <div className='space-y-4'>
            ...
            {/* 번역 시스템 정보 */}
            <div
              className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'} rounded p-2 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}
            >
              <div className='space-y-1'>
                <div>
                  🚀 <strong>Enhanced translation system active</strong>
                </div>
        ...
        <div className='mb-4 flex space-x-1 rounded-lg bg-gray-100 p-1'>
          <button
            ...
            className={`flex-1 rounded-md px-3 py-2 text-sm font-medium transition-colors ${
              activeTab === 'original' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'
            }`}
          >
            Original
          </button>
          <button
            ...
            className={`flex-1 rounded-md px-3 py-2 text-sm font-medium transition-colors ${
              activeTab === 'translated' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'
            }`}
          >
        ...
        <div className='space-y-2'>
          {transcript.length === 0 ? (
            <div className={`py-12 text-center ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              ...
              <p className='mt-2 text-sm'>The transcript will appear here in real-time</p>
            </div>
          ) : (
      ...
      {showAppPrompt && (
        <div
          className={`fixed right-0 bottom-0 left-0 p-4 ${darkMode ? 'bg-gray-800' : 'bg-white'} border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}
        >
