...
export default function SessionPage() {
  ...
  const getTabContent = (type: 'original' | 'translated') => {
    return transcript.map((line) => (
      <div
        key={`${line.id}-${type}`}
        className={`mb-3 rounded-lg p-3 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}
      >
        {showTimestamps && (
          <div
  ...
  return (
    ...
    >
      {/* Mobile Header */}
      <header
        className={`border-b ${darkMode ? 'border-gray-700 bg-gray-800' : 'bg-white'} sticky top-0 z-40`}
      >
        <div className="px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div
                className={`h-3 w-3 rounded-full ${darkMode ? 'bg-green-500' : 'bg-red-500'} animate-pulse`}
              ></div>
              <div>
                <span
