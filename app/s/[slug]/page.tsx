...
export default function PublicSessionPage() {
  ...
  const t = (key: string) => {
    ...
    const translations: Record<string, Record<string, string>> = {
      ...
      ja: {
        ...
        textOnlyModeHint:
          "💡 テキストのみモード：番号とタイムスタンプなしで純粋なテキストのみをコピーします。",
        original: "原文",
      ...
      es: {
        ...
        textOnlyModeHint:
          "💡 Modo solo texto: Copia texto puro sin números ni marcas de tiempo.",
        original: "Original",
      ...
      fr: {
        ...
        textOnlyModeHint:
          "💡 Mode texte seul: Copie le texte pur sans numéros ni horodatages.",
        original: "Original",
      ...
      de: {
        ...
        textOnlyModeHint:
          "💡 Nur-Text-Modus: Kopiert reinen Text ohne Nummern und Zeitstempel.",
        original: "Original",
    ...
    const channel = supabase
            ...
            setTranscript((prev) => {
              const updated = prev.map((line) => {
                if (
                  ...
                  (line.isTranslating ||
                    line.translated.includes("[번역 중...]"))
                ) {
                  console.log(
                    `✅ Updating line: "${line.original}" → "${cache.translated_text}"`
                  );
                  return {
                    ...line,
                    translated: cache.translated_text,
      ...
      .subscribe((status) => {
        ...
        if (status === "SUBSCRIBED") {
          console.log(
            "✅ Successfully subscribed to translation cache updates"
          );
        }
      });
    ...
    const loadSession = async () => {
      try {
        ...
        if (!sessionData) {
          ...
          setError(
            `Session not found (ID: ${slug}). The session may have ended or the link may be invalid.`
          );
          return;
        }
            ...
            if (translationEnabled && selectedLanguage !== "en") {
              try {
                ...
                if (cache) {
                  ...
                  console.log(
                    `✅ Loaded cached translation for "${t.original_text.substring(
                      0,
                      30
                    )}..."`
                  );
                  ...
                  console.log(
                    `⏳ Translation not yet cached for "${t.original_text.substring(
                      0,
                      30
                    )}..."`
                  );
                ...
                console.error(
                  `❌ Failed to load translation for "${t.original_text.substring(
                    0,
                    30
                  )}..."`,
                  err
        ...
        (payload) => {
          ...
          if (payload.eventType === "UPDATE" && payload.new) {
            ...
            if (updatedTranscript.translation_status === "completed") {
              console.log(
                `🔄 Transcript status updated to completed: "${updatedTranscript.original_text}"`
              );
  ...
  useEffect(() => {
    ...
    const reloadForNewLanguage = async () => {
      try {
        ...
        if (transcripts && transcripts.length > 0) {
          console.log(
            `🔄 Reloading ${transcripts.length} transcripts for ${selectedLanguage}`
          );
      ...
      if (isPartial) {
        ...
        setTranscript((prev) => {
          ...
          if (
            ...
            newTranscript[newTranscript.length - 1].id.includes("partial")
          ) {
  ...
  useEffect(() => {
    ...
    // 언어 변경 시에만 실행
    console.log(
      `🔄 Language changed to ${selectedLanguage}, updating translations...`
    );
    ...
    setTranscript((prev) => {
      ...
      if (selectedLanguage === "en") {
        return prev.map((line) => ({
          ...line,
          translated:
            line.original && typeof line.original === "string"
              ? line.original
              : "",
          translatedLanguage: "en",
          isTranslating: false,
    ...
    if (selectedLanguage !== "en") {
      setTimeout(async () => {
        const currentTranscripts = await new Promise<TranscriptLine[]>(
          (resolve) => {
            setTranscript((prev) => {
              resolve([...prev]);
              return prev;
        ...
        for (const line of currentTranscripts) {
          if (!line || !line.original || typeof line.original !== "string")
            continue;
      ...
      return (
        ...
        >
          ...
          >
            {type === "original"
              ? t("waitingSpeaker")
              : t("noContentTranslate")}
          </h3>
          <p className="text-sm">
            {type === "original"
              ? t("liveTranscription")
              : t("originalTranslated")}
          </p>
          {type === "original" && (
      ...
      <div className="space-y-3">
        {transcript
          ...
          .map((line, idx) => {
            ...
            return (
              <div
                key={`${type}-${line.original}-${line.translatedLanguage}-${idx}`}
                className="group"
              >
                {/* Timestamp */}
                {showTimestamps && (
                  <div
                    className={`text-xs mb-1 flex items-center space-x-2 ${
                      darkMode ? "text-gray-400" : "text-gray-500"
                    }`}
                  >
              ...
              <div className="space-y-4">
                ...
                {user && session?.host_id === user.id && (
                  <div className="text-sm text-blue-600 bg-blue-50 p-3 rounded-lg">
                    ...
                    <br />
                    Join as audience to see how your session appears to
                    attendees.
                  </div>
                )}
              </div>
  ...
  return (
    ...
    >
            ...
            <div className="space-y-3">
              <div className="space-y-3">
                ...
                <div
                  className={`text-xs ${
                    darkMode ? "text-gray-400" : "text-gray-500"
                  } p-3 rounded ${darkMode ? "bg-gray-700" : "bg-gray-50"}`}
                >
                  <div className="space-y-2">
                    <div>
                    ...
                    {translationEnabled && (
                      <div className="mt-3 pt-2 border-t border-gray-300 dark:border-gray-600">
                        ...
                        {translationStats.cached + translationStats.completed >
                          0 && (
      ...
      <ChatbotWidget
        transcript={transcript.map((line) => line.original).join("\n")}
        sessionId={sessionId || ""}
      />
    </div>
  );
