...
export function RealtimeSTT({ 
  ...
  const setupDeepgram = async () => {
    ...
    try {
      ...
      // Get microphone stream with enhanced settings for better STT accuracy
      const audioConstraints: any = {
        ...
        // Advanced Chrome-specific constraints for better STT (when available)
  ...
  const processAudioWithOpenAI = async (audioBlob: Blob) => {
    ...
    try {
      ...
      formData.append('prompt', 'This is a professional lecture or academic presentation. Please transcribe clearly with proper punctuation.') // Context prompt
  ...
  const startOpenAIWhisperSTT = (stream: MediaStream) => {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        // Enhanced audio processing for better STT accuracy
        if (audioBlob.size > 2000) { // Increased minimum size for better quality
          console.log(`🎤 Processing audio blob: ${audioBlob.size} bytes`)
          await processAudioWithOpenAI(audioBlob)
        } else {
          console.log('⚠️ Skipping small audio chunk for better quality:', audioBlob.size, 'bytes')
        }
        ...
        if (!cleanupRef.current && mediaRecorder && mediaRecorder.state === 'inactive') {
          try {
            ...
            }, 3000) // Increased to 3s for better accuracy (longer audio = better context)
          } catch (err) {
      ...
      // Start recording
      mediaRecorder.start()
      console.log('🎤 OpenAI Whisper recording started with 3s chunks for better accuracy')
      
      // Set up the 3-second interval for better STT accuracy
      setTimeout(() => {
        if (!cleanupRef.current && mediaRecorder && mediaRecorder.state === 'recording') {
          mediaRecorder.stop()
        }
