...
export function Toast({ toast, onRemove }: { toast: ToastProps; onRemove: (id: string) => void }) {
  ...
  return (
    <div
      className={`transform transition-all duration-300 ease-in-out ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'} w-auto max-w-xs ${getBgColor()} mb-2 rounded-md border px-3 py-2 shadow-md`}
    >
      <div className='flex items-center space-x-2'>
        ...
        <div className='min-w-0 flex-1'>
          <p className='text-xs leading-tight font-medium text-gray-900'>{toast.title}</p>
          {toast.message && <p className='mt-0.5 text-xs leading-tight text-gray-600'>{toast.message}</p>}
        </div>
        <button
          onClick={handleRemove}
          className='ml-1 flex-shrink-0 text-gray-400 transition-colors hover:text-gray-600'
        >
